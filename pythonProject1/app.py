...
with st.spinner("Đang tải dữ liệu..."):
    ...
    if isinstance(income_df, str):
        st.error(f"Lỗi: {income_df}")
    else:
        st.subheader("Báo Cáo Kết Quả Kinh Doanh")
        st.dataframe(income_df)

# Tạo và gửi yêu cầu phân tích
if isinstance(income_df, pd.DataFrame):
    report_data = income_df.to_string()
    prompt = (
        f"Dưới đây là Báo cáo Kết quả Kinh doanh của Ngân hàng ACB. "
        f"Phân tích tình hình doanh thu, lợi nhuận gộp, chi phí và lợi nhuận ròng trong các quý gần đây. "
        f"Dữ liệu chi tiết: {report_data}"
    )
    ...
    if st.button("Gửi yêu cầu phân tích"):
        try:
            ...
            st.error(f"Lỗi khi gửi yêu cầu tới Gemini AI: {str(e)}")
